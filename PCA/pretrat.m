function [Xp,Xtp]=pretrat(X,Xt,method)
    ...
    % input:
    % X    : matriz de refer�ncia para preprocessamento;
    % Xt   : matriz de teste;
    % method : m�todos para  preprodessar. � uma c�lula com os v�rios m�todos de preprocessamento
    %        Ex: method={'center';'auto';'snv';'msc','pareto';'minmax';'minmax2';'deriv',[7,2,1]}
    %
    % output:
...
npret=size(method,1);  % N�mero de preprocessamento
for ki=1:npret
...
function [X1,X1p] = center(Xe,Xet)
    % centrar na m�dia
    % input: Xe : matriz X para centrar na m�dia;
    %        Xet: matriz X de teste;
    % output: X1 : matriz Xe centrada na m�dia;
    %         X1p: matriz Xet centrada na m�dia;
...
function [X1,X1p] = msc(Xe,Xet)
    % corre��o de espalhamento multiplicativo
    % input: Xe : matriz X para corre�ao;
    %        Xet: matriz X de teste;
...
function [X1,X1p] = minmax(Xe,Xet)
    % Preprocessamento no intervalo [0,1] de cada vari�vel
    % input: Xe : matriz X para ser preprocessada;
    %        Xet: matriz X de teste;
    % output: X1 : matriz Xe preprocessada;
    %         X1p: matriz Xet preprocessada;
...
function [X1,X1p] = minmax2(Xe,Xet)
    % Preprocessamento no intervalo [-1, +1] de cada vari�vel
    % input: Xe : matriz X para ser preprocessada;
    %        Xet: matriz X de teste;
    % output: X1 : matriz Xe preprocessada;
    %         X1p: matriz Xet preprocessada;
...
function [X1,X1p] = derivada(Xe,Xet,para1)
    % Derivada com filtro de suaviza��o Savitsky-Golay.
    % input: Xe : matriz X para ser preprocessada;
    %        Xet: matriz X de teste;
    %      para1: vetor com [janela, grau do polin�mio, ordem da derivada]
